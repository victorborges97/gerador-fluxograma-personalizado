...
const Fluxograma = () => {
    ...
    data.periodos.map(item => {
      if(item.disciplinas) {
        item.disciplinas.map(arr => {
          totalSomaCreditos += arr.creditos;
          totalSomaChAt += arr.cargaHorariaTeorica;
          totalSomaChAp += arr.cargaHorariaPratica;
          return true
        })
      }
      return true
    })
    // totalSomaChAt += somaTeorica;
    // totalSomaChAp += somaPratica;
    // totalSomaCreditos += somaCreditos;
  }
  ...
  return (
    <div className="App" id="fluxograma">
